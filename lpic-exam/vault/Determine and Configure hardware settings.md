...
- **Module**: Module name.
- **Size**: The module is stored on the *Hard drive* but it will be loaded to the *RAM* using `modprobe` or `insmod` command and the number indicates the size of the module in *RAM* not the size of occupation on the *Hard Drive* (When it is stored in the *Hard Drive* it will be compressed but when we load it to the *RAM* it will decompress and that is the actual size of the module).
...
> In order to write and save in nano editor program use `Ctrl + o` then `Ctrl + x`
